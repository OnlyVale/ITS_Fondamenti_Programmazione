...
CREATE TABLE `editore` (
  ...
  select * from libro, editore where libro.editore_id = editore.id;
  
  select libro.titolo, editore.nome from libro, editore where libro.editore_id = editore.id;
  
  select l.titolo, e.nome from libro as l, editore as e where l.editore_id = e.id;     /*assegnazione alias ai nomi delle tabelle*/
  
  select l.titolo as 'titolo libro', e.nome as 'nome editore' from libro as l, editore as e where l.editore_id = e.id; /* assegnazione del nome delle colonne della tabella per semmplificare la lettura dell'output*/
  
  /*JOIN:*/
  
  select l.titolo as 'titolo libro', e.nome as 'nome editore'from libro as l inner join editore as e on l.editore_id = e.id; /* guardare la Vann Diagram */
